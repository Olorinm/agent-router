...
| [Matrix Server-Server API](https://spec.matrix.org/latest/server-server-api/) | domain-owned identifiers, homeserver discovery, independent operators, and explicit server policy | no Matrix rooms, event graph, PDUs, or `X-Matrix` signatures |
