# Legacy Router runtime (before Matrix)

This guide describes the retained PostgreSQL/RabbitMQ Router and Go CLI. New integrations should use the [Matrix guide](matrix.md). Use `npm run start:legacy` to run this server.
...
[![Status](https://img.shields.io/badge/status-alpha-orange.svg)](../project-status.md)
...
`send` waits and prints the A2A result by default; `--detach` returns after Task acceptance. Profiles, raw enrollment administration, credential lifecycle commands, `--json`, stdin, and environment-secret inputs remain available for operators and headless containers. See the complete [CLI guide](../guides/cli.md).
...
Start with the [deployment guide](../guides/deployment.md), then review the [security policy](../../SECURITY.md) and [configuration reference](../reference/configuration.md).
...
The normative profile is [Agent Router Federation Profile 1.0](../spec/federation-v1.md). Its design lineage and differences from Matrix, AT Protocol, and ActivityPub are documented separately in [Design lineage](../design-lineage.md).

## Documentation

| Document | Audience |
| --- | --- |
| [Local demo](../guides/local-demo.md) | first-time users |
| [Production deployment](../guides/deployment.md) | operators |
| [Register and call an agent](../guides/register-agent.md) | agent integrators |
| [CLI](../guides/cli.md) | people, scripts, and agent operators |
| [HTTP surface](../reference/http-api.md) | client and operations developers |
| [Configuration](../reference/configuration.md) | operators |
| [Federation Profile 1.0](../spec/federation-v1.md) | implementers |
| [Conformance](../conformance.md) | interoperable implementations |
| [Project status and versioning](../project-status.md) | adopters and contributors |
| [ADR 0001](../architecture/decisions/0001-federation-v1.md) | architecture reviewers |
