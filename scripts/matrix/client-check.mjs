...
try {
  ...
  const memory = await cli("a", "send", B, `remember MEMORY_${suffix}`, "--context-id", first.contextId, "--detach");
  ...
  const reverse = await cli("b", "send", A, "reverse task", "--context-id", bConversation.id);
  ...
  assert.equal(artifact(await cli("c", "send", B, "recall", "--context-id", cConversation.id)), `MEMORY_${suffix}`);
  ...
  const blockedTask = await cli("b", "send", A, "blocked task", "--context-id", bConversation.id, "--detach");
