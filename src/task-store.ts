...
export class PostgresTaskStore implements TaskStore {
  private readonly pendingDeliveries = new Map<string, DeliveryEnvelope>();
  ...
  stageDelivery(envelope: DeliveryEnvelope): void {
    this.pendingDeliveries.set(deliveryKey(envelope), structuredClone(envelope));
  }

  async save(task: Task, context: ServerCallContext): Promise<void> {
    const scope = scopeFromContext(context);
    const stagedKey = taskKey(scope, task.id);
    const publicTask = stripRouterMetadata(task);
    let changed = false;
    try {
      changed = await withTransaction(this.pool, async (client) => {
        ...
        if (existing && isDeepStrictEqual(stripRouterMetadata(existing.task), publicTask)) return false;

        const envelope = this.pendingDeliveries.get(stagedKey) ?? readEnvelope(task);
      ...
      this.pendingDeliveries.delete(stagedKey);
    }
    if (changed && this.pushNotificationSender) {
      void this.pushNotificationSender
        .send({ payload: { $case: "task", value: structuredClone(publicTask) } }, context, structuredClone(publicTask))
  ...
  async load(taskId: string, context: ServerCallContext): Promise<Task | undefined> {
    ...
    return row ? stripRouterMetadata(row.task) : undefined;
  }
  ...
  async findByMessage(agentId: string, messageId: string, context: ServerCallContext): Promise<Task | undefined> {
    ...
    return result.rows[0]?.task ? stripRouterMetadata(result.rows[0].task) : undefined;
  }
  ...
  async list(params: ListTasksRequest, context: ServerCallContext): Promise<ListTasksResponse> {
    ...
    const [result, totalResult] = await Promise.all([
      this.pool.query<TaskRow & { task_id: string }>(
      ...
      this.pool.query<{ total: string }>(
        `SELECT count(*)::text AS total FROM tasks WHERE ${totalWhere.join(" AND ")}`,
        totalValues,
      ),
    ]);
    ...
    const tasks = selected.map((row) => {
      const task = stripRouterMetadata(row.task);
    ...
    return {
      ...
      pageSize,
      totalSize: Number(totalResult.rows[0]?.total ?? 0),
    };
  }
}
...
function messageIdempotencyScope(scope: TaskScope, agentId: string): string {
  ...
  return federation
    ? JSON.stringify(["federation", federation.issuer, agentId])
    : JSON.stringify(["local", scope.tenant, scope.ownerPrincipalId, agentId]);
}

function taskKey(scope: TaskScope, taskId: string): string {
  return JSON.stringify([scope.tenant, scope.ownerPrincipalId, taskId]);
}

function deliveryKey(envelope: DeliveryEnvelope): string {
  return taskKey(
    { tenant: envelope.tenant, ownerPrincipalId: envelope.ownerPrincipalId },
    envelope.routerTaskId,
  );
}

function stripRouterMetadata(task: Task): Task {
  const copy = structuredClone(task);
  if (!copy.metadata) return copy;
  delete copy.metadata[ROUTER_METADATA_KEY];
  delete copy.metadata.remoteTaskId;
  delete copy.metadata.remoteContextId;
  if (Object.keys(copy.metadata).length === 0) copy.metadata = {};
  return copy;
}
